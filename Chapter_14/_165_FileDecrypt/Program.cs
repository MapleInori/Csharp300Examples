...
namespace _165_FileDecrypt
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //DES加/解密算法服务对象
            FileStream fileEncrypt = File.OpenRead("300 classic examples.encrypt");
            //打开加密的文件
            ...
            CryptoStream cryptoStream = new CryptoStream(memoryStream, des.CreateDecryptor(),
                    CryptoStreamMode.Write);                   //创建解密流对象
            cryptoStream.Write(fileArray, 0, fileArray.Length); //将解密后的文件内容写入内存流中
            cryptoStream.FlushFinalBlock();

            FileStream file = File.Create("300 classic examples.txt");  //创建原文件将解密的
                                                                        //文件内容写入到文件中
