    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            CryptoStream cryptoStream = new CryptoStream(memoryStream, des.CreateEncryptor(),
                    CryptoStreamMode.Write);                   //创建加密流对象
            cryptoStream.Write(fileArray, 0, fileArray.Length); //将内容加密后的文件写入内存流中
            cryptoStream.FlushFinalBlock();
            FileStream fileEncrypt = File.Create("300 classic examples.encrypt");
            //加密后的文件将加密的文件内容写入文件中
