namespace _038_ProvinceCity
{
    internal class Program
    {
        ...
        {    //创建省份列表
            ...
            JiangSu.Add("南京");                                 //添加城市
            JiangSu.Add("苏州");                                 //添加城市
            foreach (KeyValuePair<string, List<string>> province in provinces)   //遍历省份信息
            {
                Console.WriteLine("{0}省：", province.Key);
                foreach (string city in province.Value)                       //遍历城市信息
                {
            ...
            {
                foreach (string city in provinces[pro])              //遍历城市信息
                    Console.WriteLine("\t{0}市", city);
            }
            else
            {
